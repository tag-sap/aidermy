...
def _prompt(
    ...
    return (
        "Ты — косметолог. Напиши понятное объяснение результата подбора косметики.\n\n"
        f"Продукт: {product_name}\n"
        f"Тип кожи: {skin_type or 'не указан'}\n"
        f"Проблемы: {', '.join(concerns) or 'не указаны'}\n"
        f"Итоговая совместимость (рассчитана алгоритмом): {score}%\n"
        f"Положительные факторы: {pos}\n"
        f"Отрицательные факторы: {neg}\n\n"
        "Объясни: почему средство получило именно этот процент, какие компоненты дали "
        "основной вклад, что может быть полезно именно этому пользователю и на что "
        "стоит обратить внимание.\n"
        "ВАЖНО: не предлагай другой процент и не пересчитывай оценку. Процент "
        f"зафиксирован алгоритмом и равен {score}%. Верни только текст на русском, 2-4 предложения."
    )
